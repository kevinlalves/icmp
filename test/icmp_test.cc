...
TEST(IcmpTest, Encode) {
  ...
  // extra data stored in pure bytes
  ...
  data = {0x74, 0x50, 0x76, 0x81, 0x53, 0xf8, 0xe6, 0}; // random 8 byte number

  packet->SetPayload(data);
  byte_array = {0x8, 0, 0xb3, 0x1b, 0x29, 0x81, 0xf6, 0x97, 
                0x74, 0x50, 0x76, 0x81, 0x53, 0xf8, 0xe6, 0}; // payload at second line
  EXPECT_EQ(packet->Encode(), byte_array);
  byte_array.clear();
  data.clear();
  delete packet;
  ...
  data = {0x58, 0xd8, 0xbe, 0xeb, 0x80, 0x54, 0xfa, 0};
  packet->SetPayload(data);
  byte_array = {0x8, 0, 0xb6, 0xd3, 0x90, 0xca, 0x1e, 0x48, 
                0x58, 0xd8, 0xbe, 0xeb, 0x80, 0x54, 0xfa, 0};
  EXPECT_EQ(packet->Encode(), byte_array);
...
TEST(IcmpTest, Decode) {
  ...
  EXPECT_NE(packet.Encode(), byte_array);
}
